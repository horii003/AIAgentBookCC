...
AG-001 からツールとして呼び出される交通費精算申請書作成専門エージェント。
移動情報の収集・交通費計算・承認・申請書生成を担当する。
"""
import logging
...
logger = logging.getLogger("agents.transport_agent")
...
def _build_prompt(applicant_name: str, application_date: str) -> str:
    ...
    Returns:
        str: システムプロンプト全文
    """
    return build_transport_prompt(applicant_name, application_date)
...
def handle_transport_expense_application(query: str, tool_context: ToolContext) -> str:
    ...
    logger.info(
        f"交通費精算申請エージェント呼び出し開始: session_id={session_id}, "
        f"クエリ先頭={query[:50]}"
    )
    ...
    try:
        ...
        logger.error(
            f"交通費精算申請エージェント: バリデーションエラー: session_id={session_id}",
            exc_info=True,
        )
        return ErrorHandler.handle_validation_error(e)

    if session_id not in _agent_instances:
        logger.info(f"交通費精算申請エージェント: インスタンス生成: session_id={session_id}")
        ...
        logger.info(f"交通費精算申請エージェント: インスタンス再利用: session_id={session_id}")
    ...
    try:
        ...
        logger.warning(
            f"交通費精算申請エージェント: ループ上限到達: session_id={session_id}, "
            f"クエリ先頭={query[:50]}"
        )
        ...
        if "ContextWindowOverflow" in type(e).__name__:
            logger.warning(
                f"交通費精算申請エージェント: コンテキストウィンドウ超過: session_id={session_id}"
            )
            return ErrorHandler.handle_context_window_error(e)
        elif "MaxTokensReached" in type(e).__name__:
            logger.warning(
                f"交通費精算申請エージェント: トークン上限超過: session_id={session_id}"
            )
            return ErrorHandler.handle_max_tokens_error(e)
        elif isinstance(e, RuntimeError):
            logger.error(
                f"交通費精算申請エージェント: ランタイムエラー: session_id={session_id}, "
                f"クエリ先頭={query[:50]}",
                exc_info=True,
            )
            ...
            logger.error(
                f"交通費精算申請エージェント: 想定外エラー: session_id={session_id}, "
                f"クエリ先頭={query[:50]}",
                exc_info=True,
            )
