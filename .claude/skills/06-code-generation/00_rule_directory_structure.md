...
| 評価テスト | `evals/` | `eval_{evaluation_name}.py` | `eval_goal_success_rate.py` |
| ガードレール | `guardrails/` | `guardrails_cloudformation.yaml` | `guardrails_cloudformation.yaml` |
| ハンドラー | `handlers/` | `{機能名}_handler.py`  | `error_handler.py` |
| フック | `hooks/` |  `{機能名}_hook.py` | `loop_control_hook.py` |
...
```
[上位層]
  agents/     → tools/, prompt/, handlers/, hooks/, session/, config/, models/, knowledge/
  main.py     → agents/, handlers/, hooks/, config/

[中間層]
  tools/      → models/, handlers/, hooks/
  prompt/     → knowledge/
...
| テンプレートファイル | `data/template/` | 出力生成（Excel・PDF等）に使用するテンプレートファイル。ツールから参照する。 |
| サンプルデータ | `sample/` | テスト・デモ用のファイル。本番動作には不要。 |
| セッション永続化データ | `data/sessions/` | 実行時に自動生成。セッションIDごとのサブディレクトリに格納。 |
...
| 出力種別 | 配置先 | ファイル命名規則 | 例 |
|:---|:---|:---|:---|
| 出力ファイル（Excel/PDF等） | `data/output/` | `{出力種別}_{YYYYMMDD_HHMMSS}.{拡張子}` | `報告書_20260210_143022.xlsx` |
| ログファイル | `logs/` | `agent.log` | `agent.log` |
| セッションデータ | `data/sessions/` | `session_{セッションID}/` | `session_20260210_143022_a1b2c3d4/` |
